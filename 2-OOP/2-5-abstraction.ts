 ...
 * 
 * 동일한 오브젝트의 인스턴스여도, 어떤 인터페이스를 생성자로 받아오냐에 따라
 * 해당 인터페이스에 맞게 제한된 범위에서만 인스턴스에서 사용가능한 메소드를 규격화할 수 있다.
 * 해당 오브젝트는 인터페이스가 어떻게 구현되어있고 얼마나 많은 메소드가 있는지는 궁금하지 않다.
 * 본인에게 연결된 인터페이스만 어떻게 사용하면 되는지 알면된다.
 */
{
	type CoffeeCup = {
		shots : number;
